...
// File: contracts/BIFI/interfaces/bdollar/IBdoRewardPool.sol
...
// File: contracts/BIFI/strategies/BDollar/StrategyBdoLP.sol
...
contract StrategyBdoLP is Ownable, Pausable {
    ...
    constructor(address _lpPair, uint8 _poolId, address _vault, address _strategist) public {
        ...
        IERC20(lpPair).safeApprove(bdoRewardPool, uint(-1));
        IERC20(bdo).safeApprove(unirouter, uint(-1));
    ...
    function chargeFees() internal {
        ...
        IPancakeRouter(unirouter).swapExactTokensForTokens(toWbnb, 0, bdoToWbnbRoute, address(this), now.add(600));
    ...
    function addLiquidity() internal {   
        ...
        if (lpToken0 != bdo) {
            IPancakeRouter(unirouter).swapExactTokensForTokens(bdoHalf, 0, bdoToLp0Route, address(this), now.add(600));
        }

        if (lpToken1 != bdo) {
            IPancakeRouter(unirouter).swapExactTokensForTokens(bdoHalf, 0, bdoToLp1Route, address(this), now.add(600));
        }
